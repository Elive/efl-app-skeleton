...
static void skeleton_test_basic(TCase *tc);

static const struct {
   const char *name;
   void (*build)(TCase *tc);
} tests[] = {
  { "basic", skeleton_test_basic }
};

START_TEST(skeleton_initialization)
{
   fail_if(skeleton_init() != 1);

   skeleton_library_call();

   fail_if(skeleton_shutdown() != 0);
}
END_TEST

static void
skeleton_test_basic(TCase *tc)
{
   tcase_add_test(tc, skeleton_initialization);
}
